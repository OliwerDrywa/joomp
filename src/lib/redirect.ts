import { deserializeCommandTree, executeCommand } from "./commands";
...
export default function redirect() {
  ...
  const tree = deserializeCommandTree(bangURLs);

  const actions = executeCommand(q, tree);

  for (const a of actions) {
    if (a === "this://close") continue;
    window.location.replace(a);
  }

  if (!actions.at(-1)?.startsWith("https://")) {
